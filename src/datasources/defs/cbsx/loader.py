...
over the partition range. Direct identifiers (names, phone/email, national-ID
and passport numbers, dates of birth) are dropped as records stream through —
only an analytical, non-identifying subset is written to the raw bucket.
"""
...
def _client() -> RESTClient:
    ...
    return RESTClient(
        base_url=BASE_URL,
        headers={"x-api-key": dlt.secrets["datasources.cbsx.api_key"]},
...
@dlt.source(name="cbsx", max_table_nesting=0)
def cbsx_source():
